...
class PBParser
{
    ...
    // different types
    var $scalar_types = array('double'=>'PBDouble', 'float'=>'PBFloat', 'int32' => 'PBInt', 'int64' => 'PBInt',
    ...
    private function _create_class_constructor($classfile, &$string, $classname)
    {
        ...
        foreach($classfile as $field)
        {
            ...
            $string .= '    $this->fieldnames["' . $field['value']['value'] . '"] = "' . $field['value']['name'] . '"' . ";\n";
    ...
    private function _parse_message_type(&$string, $m_name, $path = '')
    {
        ...
        while (strlen($string) > 0)
        {
            ...
            else if( strtolower($next) == 'import' )
            {
                ...
                if( !$match )
                  throw new Exception( 'Malformed include' );
