 ...
 *
 */
class Meigee_ThemeOptionsUnique_Model_Import extends Mage_Core_Model_Abstract
{
    protected $settings;
    private $_file = '/app/code/local/Meigee/ThemeOptionsUnique/etc/import/';
    ...
    public function getCorrectSet ($param) {
        $this->_file .= $param . '.xml';
        ...
        if ($this->settings->getNode('cms/pages')){
            $this->setupPages();
        }
        if ($this->settings->getNode('cms/blocks')) {
            $this->setupBlocks();
        }
    }
