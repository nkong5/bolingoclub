...
//				IWD.OPC.recheckAgree();
		...
		showLoader: function(){
			...
			$j_opc('.opc-index-index .content-wrapper').addClass('overlay');
		},
		
		hideLoader: function(){
			setTimeout(function(){
				$j_opc('.opc-ajax-loader').hide();
				$j_opc('.opc-index-index .content-wrapper').removeClass('overlay');
			},600);
		},
...
IWD.OPC.Agreement ={
		...
		init: function(){
			...
			$j_opc(document).on('click', '.view-agreement', function(e){
				e.preventDefault();
				$j_opc('.opc-col-right #modal-agreement').addClass('md-show');
				$j_opc('.opc-index-index .content-wrapper').addClass('overlay');
				
				var id = $j_opc(this).data('id');
				var title = $j_opc(this).html();
				var content = $j_opc('.opc-col-right #agreement-block-'+id).html();
				
				$j_opc('.opc-col-right #agreement-title').html(title);
				$j_opc('.opc-col-right #agreement-modal-body').html(content);
			});
