...
/* shop by move */
function shopByMove(isMobile){
	block = jQuery('.block.block-vertical-nav');
	if(isMobile){
		if(!jQuery('.col-main .sidebar').length){
			destLoc = jQuery('header.page-title.category-title');
			destLoc.after('<aside class="sidebar sidebar-left"><div class="indent" /></aside>');
			block.appendTo('.col-main .sidebar .indent');
		}
	}else{
		if(jQuery('.col-main .sidebar').length){
			sourceLoc = jQuery('.main-container > aside.sidebar .indent');
			block.prependTo(sourceLoc);
			jQuery('.col-main .sidebar').remove();
		}
	}
}
...
function mobileMenuBlocks(){
	...
	headerOffset = jQuery('.header-wrapper').offset().top;
	if(jQuery(document.body).width() < 767 && !jQuery(document.body).hasClass('ajax-index-options')){
		if(!jQuery('header#header').hasClass('floating')){
			if(!jQuery('.header-top-right .header-search').hasClass('active')) {
				mobileMenu.css('margin-top', mobileMenuTopBlock.outerHeight(true));
				mobileMenuTopBlock.css({
					'top' : mobileMenuButton.innerHeight() + mobileMenuButton.offset().top - headerOffset + 'px',
					'width' : mobileMenuButton.outerWidth(true)
				});
			}
		}
	} else {
		mobileMenu.css('margin-top', '');
		...
		if(jQuery(document.body).width() > 977) {
			if(!jQuery('header#header').hasClass('header-2') && !jQuery('header#header').hasClass('header-3')) {
						...
						menuButton.off().on('click', function(){
							if(menu.hasClass('open')){
								...
								if(jQuery('.breadcrumbs-wrapper').length) {
									menu.parent('.header-bottom-left').animate({
										'height' : menu.children().height() + menuButton.outerHeight() + (menuIndent*2),
										'margin-bottom' : -((menu.children().height() + menuButton.outerHeight() + (menuIndent*2)) - slider.height())
									}, 300);
								} else {
									menu.parent('.header-bottom-left').animate({
										'height' : menu.children().height() + menuButton.outerHeight() + (menuIndent*2),
										'margin-bottom' : -((menu.children().height() + (menuIndent*2)) - slider.height())
									}, 300);
								}
							}
				...
				jQuery('#nav-wide .menu-wrapper').each(function(){
					if(jQuery(this).find('.transparent').length){
						block = jQuery(this).parent();
						blockHeight = block.position().top;
						jQuery(this).find('.transparent').css('height', blockHeight);
						jQuery(this).on('mouseover', function(e){
							if (jQuery(e.target).hasClass('transparent') == 0) {
								jQuery(this).find('.transparent').addClass('non-active');
							}
						});
						jQuery(this).on('mouseout', function(e){
							jQuery(this).find('.transparent').removeClass('non-active');
						});
					}
				});
			} else {
				if(jQuery('body').hasClass('accordion-menu')){
					setTimeout(function(){
						if(menuButton.parent().css('display', 'none').length == 0) {
							...
							try{
								menu.css('height', menu.find('li.level-top').innerHeight());
							}catch(e){}
						};
						try{
							if(menu.innerHeight() >= menuCHeight) {
...
function productImgPos(){
	images = jQuery('.products-grid .product-img-box img');
	...
	function launcher(){
		jQuery('.products-grid').each(function(){
			productWidth = jQuery(this).find('li.item').width();
			columnsCount = Math.round(jQuery(this).width()/productWidth);
			items = jQuery(this).find('.product-img-box');
			groupsCount = items.size()/columnsCount;
			ratio = 1;
			for(i=0; i<groupsCount; i++){
				currentGroupe = items.slice((i*columnsCount), (columnsCount*ratio));
				setHeight(currentGroupe);
				ratio++;
			}
		});
	}
	
	function setHeight(imgBox){
		var maxHeight = 0;
		imgBox.each(function(){
			height = jQuery(this).find('.product-image img').height();
			if(height > maxHeight){
				maxHeight = height;
			}
		});
		imgBox.css('min-height', maxHeight+'px');
	}
}

function wideMenuDropdown(){
	isActive = jQuery('.nav-wide#nav-wide').hasClass('columns-active');
	if(jQuery(document.body).width() > 977){
		if(!isActive){
			jQuery('.nav-wide#nav-wide li.level0 ul.level0').each(function(){
				var maxColumnHeight = 314;
				var currentHeight = 0;
				var firstIndex = 0;
				var lastIndex = 0;
				var indexStore = [];
				var resultHtml = '';
				allItems = jQuery(this).find('li.level1');
				...
				allItems.each(function(index){
					...
					if(currentHeight > maxColumnHeight){
						...
						lastIndex = index;
						indexStore.push([firstIndex, lastIndex-1]);
						currentHeight = jQuery(this).height();
						
						if(index == maxIndex){
							indexStore.push([index, index]);
						}
					}else if(index == maxIndex){
						indexStore.push([lastIndex, index]);
					}
				});
				
				if(indexStore.length){
					for(i=0; i<indexStore.length; i++){
						resultHtml += '<ul class="level0">';
						
						itemsRange = allItems.slice(indexStore[i][0], indexStore[i][1]+1);
						itemsRange.each(function(){
							resultHtml += '<li class="level1">' + jQuery(this).html() + '</li>';
						});
						resultHtml += '</ul>';
					}
					jQuery(this).after(resultHtml).remove();
					jQuery('.nav-wide#nav-wide').addClass('columns-active');
				}
			});
		}
		...
		jQuery('.nav-wide#nav-wide .menu-wrapper').each(function(){
			...
			wrapperWidth = jQuery('.header-bottom-wrapper').width();
			parentPos = jQuery(this).parents('li.level0').position().left;
			
			if((dropdownWidth + parentPos) > wrapperWidth){
				difference = wrapperWidth - parentPos;
				parentPos = parentPos - (dropdownWidth - difference);
				jQuery(this).css('left', parentPos);
			}else{
				jQuery(this).css('left', parentPos);
			}
		});
...
function productTitleLines(){
	items = jQuery('.products-grid .product-name');
	var lineHeight = parseInt(items.eq(1).find('a').css('line-height'));
	var targetHeight = lineHeight * 2; //set product name lines to: 2 lines
	targetHeight += targetHeight * 0.1; //+10% to height
	
	items.each(function(){
		if(jQuery(this).hasClass('transformed')){
			jQuery(this).removeClass('transformed');
			jQuery(this).find('a').html(jQuery(this).attr('data-original-name'));
		}
  ...
  /* Coin Slider */
	...
	if (jQuery('#zoom').length) {
		...
		function zoomSwitcher(){
			jQuery('#wrap').removeClass('no-zoom');
			if(jQuery('#zoom').width() != jQuery('#zoom img').width()){
				jQuery('#wrap').addClass('no-zoom');
			}
		}
		function zoomLoader(){
			if(!jQuery('#wrap .cloud-zoom-loading').length){
				zoomSwitcher();
			}else{
				setTimeout(function(){
					zoomLoader();
		...
		zoomLoader();
		jQuery('.cloud-zoom-gallery').on('click mouseover', function(){
			setTimeout(function(){
				zoomLoader();
			}, 500);
		});
	...
	function mobileDevicesMenu(action){
		if(action == 'reset'){
			...
			jQuery(".nav-container:not('.mobile') .nav li a, .nav-container:not('.mobile') .nav-wide li a").each(function(){
				if(jQuery(this).parents('.nav-wide').length){
					if(jQuery(this).parent('.level-top').length || (jQuery(this).parents('.menu-wrapper.default-menu').length && jQuery(this).next('ul').length)){
						linkAction(jQuery(this));
					}
				}else{
					linkAction(jQuery(this));
				}
		...
		case 'animate':
		   if(!jQuery('.nav-container').hasClass('mobile')){
				...
				jQuery('.menu-button').on('click', function(event){
					event.stopPropagation();
					jQuery('.nav-container, .nav-container > ul').toggleClass('active');
					...
					if(jQuery('header#header').hasClass('floating')){
						...
						dropdownTop = jQuery('header#header .header-top').height();
						menuHeight = windowHeight - dropdownTop - 10;
						jQuery('.nav-container > ul').css('max-height', menuHeight);
					}
			   ...
			   jQuery('.nav-container > ul a').each(function(){
					if(jQuery(this).next('ul').length || jQuery(this).next('div.menu-wrapper').length){
						...
						jQuery(this).prev('.menu-item-button').on('click', function(){
							jQuery(this).next('a').toggleClass('open');
							jQuery(this).toggleClass('active');
				...
				jQuery('.nav-container > ul a').on('click.dropdownLinks', function(event){
					if(jQuery(this).next('ul').length || jQuery(this).next('div.menu-wrapper').length){
						if(!jQuery(this).hasClass('open')){
							event.preventDefault();
							//jQuery(this).addClass('open');
							jQuery(this).prev('.menu-item-button').trigger('click');
						}
					}
				});
		   }
	...
	productTitleLines();
	mobileSidebar();
	jQuery(window).resize(function(){toDo(); replacingClass(); more_view_set_height(); wishlist_set_height(); related_set_height(); WideMenu(); headerCustomer(); mobileMenuBlocks(); sliderBanners(); backgroundWrapper(); sidebarMenu(); productTitleLines(); mobileSidebar();});
	/* Responsive */
...
/* Messages button */
	...
	if(jQuery('#header .links').hasClass('custom')){
		...
		wishlistLink.appendTo(jQuery('header#header .header-top .first-line .grid_5'));
	}
