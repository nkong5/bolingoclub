...
function titleDivider(){
	setTimeout(function(){
		jQuery('.widget-title, #footer .footer-block-title, aside.sidebar .block-title, header.rating-title, .box-reviews .rating-subtitle, .block-related .block-title, .product-options-title, .cart header, #login-holder .page-title, .op_block_title, .quick-view-title, .block-poll .block-title, .widget-latest .page-title').each(function(){
			...
			if (divider_width > 15) {
				...
				if(jQuery(this).hasClass('widget-title') && jQuery(this).next('.slider-container').length || jQuery(this).next('.brands-wrapper').length){
					if(jQuery(this).next().find('.widget-slider-buttons').length == 0) {
						jQuery(this).find('.right-divider').css('width', divider_width - 40);
					} else {
						jQuery(this).find('.right-divider').css('width', divider_width);
					};
				} else if (jQuery(this).parents().hasClass('related-wrapper-bottom')) {
					jQuery(this).find('.right-divider').css('width', divider_width - 110);
				} else {
					jQuery(this).find('.right-divider').css('width', divider_width);
				}
			} else {
...
jQuery(window).load(function() {
  ...
  /* Coin Slider */
	...
	titleDivider();
	mobileSidebar();
	jQuery(window).resize(function(){toDo(); replacingClass(); more_view_set_height(); wishlist_set_height(); related_set_height(); WideMenu(); headerCustomer(); mobileMenuBlocks(); sliderBanners(); backgroundWrapper(); sidebarMenu(); productTitlesHeight(); titleDivider(); mobileSidebar()});
	/* Responsive */
	...
	if(jQuery('body').hasClass('floating-header')){
		setTimeout(function(){
			...
			setTimeout(function(){
				sliderHeight = jQuery('#header').find('.header-slider-holder').height();
			}, 1000);
		}, 200);
	}
});
