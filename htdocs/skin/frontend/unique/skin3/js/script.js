...
function logoResize(){
	jQuery('header#header h2.logo, header#header h2.small-logo').each(function(){
		...
		jQuery(this).parents('.grid_6').css('position', 'static');
		...
		jQuery(this).parents('.grid_6').attr('style', '');
	});
}
  ...
  /* Coin Slider */
	...
	function headerCustomer() {
		if(jQuery('#header .customer-name').length && jQuery('#header .links').hasClass('custom')){
			...
			var linksTop = custName.outerHeight(true) + parseFloat(jQuery('#header .header-blocks').css('padding-top'));
	...
	function socialLinksDivider(){
		setTimeout(function(){
			jQuery('.social-links-wrapper').each(function(){
				links_container_width = jQuery(this).width();
				links_width = jQuery(this).find('.social-links').innerWidth();
				divider_width = ((links_container_width - links_width-2)/2);
				if (divider_width > 15) {
	...
	mobileSidebar();
	socialLinksDivider();
	jQuery(window).resize(function(){toDo(); replacingClass(); more_view_set_height(); wishlist_set_height(); related_set_height(); WideMenu(); headerCustomer(); mobileMenuBlocks(); sliderBanners(); backgroundWrapper(); sidebarMenu(); productTitlesHeight(); mobileSidebar(); socialLinksDivider()});
	/* Responsive */
