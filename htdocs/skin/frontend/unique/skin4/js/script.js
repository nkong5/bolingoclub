...
function logoResize(){
	jQuery('header#header h2.logo, header#header h2.small-logo').each(function(){
		...
		if(jQuery('#header.floating').length && pixelRatio > 1 && jQuery('#header .resized-logo').length) {
			jQuery(this).find('a.logo img').attr('style', 'width: auto; max-width: none;').css({
...
/* Mobile Sidebar on Product Page */
	...
	function headerSliderIndent() {
		if(jQuery('.header-slider-holder').length){
			if(jQuery('body').hasClass('boxed-layout') || jQuery('.header-slider-holder .header-slider-container').hasClass('boxed-slider')) {
				jQuery('.header-slider-holder').addClass('boxed');
			}
			if (jQuery(document.body).width() > 977) {
				setTimeout(function(){
					header = jQuery('.header-top');
					menu = jQuery('.header-top .nav-container:not(".open")');
					var headerHeight = header.outerHeight() - menu.outerHeight() + jQuery('.header-top .nav-container').children().outerHeight();
					jQuery('.header-slider-holder').css({
						'top' : -headerHeight,
		...
		if(jQuery(document.body).width() > 977) {
			if(!jQuery('header#header').hasClass('header-2') && !jQuery('header#header').hasClass('header-3')) {
				...
				if(jQuery('body').hasClass('accordion-menu')){
					if(jQuery('.header-slider-holder').length){
						menu.css('margin-bottom', - menuCHeight - menuButton.outerHeight());
					} else {
					...
					setTimeout(function(){
						if(menuButton.parent().css('display', 'none').length == 0) {
							...
							jQuery('#header .nav-container').css('height', jQuery('#header .nav-container').find('li.level-top').innerHeight());
						};
						if(jQuery('#header .nav-container').innerHeight() >= menuCHeight) {
							menuButton.parent().off().css('display', 'none');
						} else {
							menuButton.parent().css({
								'display' : 'block',
...
jQuery(window).load(function() {
  ...
  /* Coin Slider */
			...
			jQuery('.background-wrapper').each(function(){
				...
				if(jQuery(this).parent('.text-banner').length){
					...
					if(jQuery(document.body).width() > 1278) {
						...
						blockIndent = (blockHeight - textHeight) * 0.6;
						blockSecondIndent = (blockHeight - textHeight) * 0.4;
						bg.css('height', blockHeight);
						block.css({
							'padding-top' : blockIndent,
							'padding-bottom' : blockSecondIndent
						});
					} else {
						block.attr('style', '');
						bg.css('height', 'auto');
	...
	jQuery('.twitter-timeline').contents().find('head').append('<style>body{color: #aaa} body .p-author .profile .p-name{color: #666}</style>');
	...
	if((navigator.userAgent.match(/iPhone/i)) || (navigator.userAgent.match(/iPod/i)) || (navigator.userAgent.match(/iPad/i))){
		...
		jQuery('input, textarea').on('focusin focusout', function(){
	...
	function stickyHeader(){
		if(jQuery('body').hasClass('floating-header')){
			setTimeout(function(){
				if(jQuery(document.body).width() > 977) {
					if(jQuery('.header-slider-holder').length == 0) {
						headerHeight = jQuery('#header').height();
					} else {
						headerHeight = jQuery('.header-slider-holder .item a img').height();
					}
				} else {
					headerHeight = jQuery('#header').height();
				}
	...
	headerSliderIndent();
	stickyHeader();
	jQuery(window).resize(function(){toDo(); replacingClass(); more_view_set_height(); wishlist_set_height(); related_set_height(); WideMenu(); headerCustomer(); mobileMenuBlocks(); sliderBanners(); backgroundWrapper(); sidebarMenu(); productTitlesHeight(); mobileSidebar(); headerSliderIndent(); stickyHeader()});
	/* Responsive */
